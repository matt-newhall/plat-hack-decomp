...
static BOOL BtlCmd_ChangeStatStage(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    if (stageChange > 0) {
        ...
        if (battleCtx->attacker != battleCtx->sideEffectMon
            && Battler_Side(battleSys, battleCtx->attacker) != Battler_Side(battleSys, battleCtx->sideEffectMon)) {
...
static BOOL BtlCmd_TryTeleport(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    } else if ((BattleSystem_BattleType(battleSys) & BATTLE_TYPE_DOUBLES|BATTLE_TYPE_2vs2|BATTLE_TYPE_AI)
        && (Battler_Side(battleSys, battleCtx->attacker) != BATTLER_US)
...
static BOOL BtlCmd_ResetSleepTurns(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    for (int i = 0; i < maxBattlers; i++) {
        if (Battler_Side(battleSys, i) == BATTLER_US) {
