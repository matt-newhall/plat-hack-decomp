...
static void BattleController_NeutralizingGasPreSwitch(BattleSystem *battleSys, BattleContext *battleCtx);
...
static const BattleControlFunc sBattleControlCommands[] = {
    ...
    BattleController_NeutralizingGasPreSwitch,
};
...
static BOOL BattleController_DecrementPP(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    if (ATTACKER_SELF_TURN_FLAGS.skipPressureCheck == FALSE && battleCtx->defender != BATTLER_NONE) {
        if (battleCtx->moveTemp == MOVE_SNATCH) {
 ...
 * the rest of the user's move.
 */
static BOOL BattleController_TriggerPranksterImmunity(BattleSystem *battleSys, BattleContext *battleCtx)
{
    int result = STATE_PROCESSING;
    ...
    do {
        ...
        case IMMUNITY_ABILITY_STATE_CHECK:
            if (Battler_Ability(battleCtx, battleCtx->attacker) == ABILITY_PRANKSTER
                ...
                && Battler_Side(battleSys, battleCtx->attacker) != Battler_Side(battleSys, battleCtx->defender)) {
